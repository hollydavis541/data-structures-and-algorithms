...
const notInFirstArray = (forbiddenValues, arr) => {
  const newArr = [];
  arr.filter( value => {
    if (!forbiddenValues.includes(value)) newArr.push(value);
  });
  return newArr;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  const newArr = [];
  arr.filter( value => {
    if (value.baseStat > minBaseStat) newArr.push(value);
  });
  return newArr;
};
...
const getStatName = (arr, minBaseStat) => {
  const newArr = [];
  arr.filter( value => {
    if (value.baseStat > minBaseStat) newArr.push(value.stat.name);
  });
  return newArr;
};
...
const evenOddNumericValues = (arr) => {
  ...
  const newArr2 = [];
  arr.filter( value => {
    if (!isNaN(value)) newArr.push(value);
  })
  newArr.map( value => {
    if (value % 2 === 0) {
      newArr2.push('even');
